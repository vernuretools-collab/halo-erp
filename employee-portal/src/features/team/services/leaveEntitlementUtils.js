...
export const leaveMatchesEmployeeFilter = (leave, filter = {}) => {
  const { employeeId, employeeEmail, employeeName, uid, identityIds } = filter
  if (!leave) return false
  const ids = new Set(
    [employeeId, uid, ...(Array.isArray(identityIds) ? identityIds : [])]
      .filter((value) => value != null && value !== '')
      .map(String)
  )
  if (leave.employeeId && ids.has(String(leave.employeeId))) return true
  if (leave.uid && ids.has(String(leave.uid))) return true
